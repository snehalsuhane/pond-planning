...
{\Large Phase 1: Contour Map Analysis \& Catchment Estimation\par}\vspace{2cm}
{\large\bfseries Snehal Suhane\par}\vspace{0.3cm}
{\normalsize September 2026\par}\vspace{2cm}
\hrule\vspace{0.4cm}
\begin{tabular}{ll}
\textbf{GitHub:}&\url{https://github.com/snehalsuhane/pond-planning}\\[4pt]
\textbf{API Base URL:}&\texttt{http://localhost:5000}\\[4pt]
...
\begin{center}
\url{https://github.com/snehalsuhane/pond-planning}
\end{center}

The repository is fully self-contained:
\begin{itemize}
\item \textbf{\texttt{app.py}} --- Flask application factory and server entry point
\item \textbf{\texttt{routes/contour.py}} --- HTTP route handler, file validation
\item \textbf{\texttt{services/contour\_service.py}} --- Pipeline orchestration
\item \textbf{\texttt{analysis/}} --- Terrain, DEM, hydrology, pond ranking, catchment
\item \textbf{\texttt{utils/}} --- KML/KMZ parser, UTM projection
\item \textbf{\texttt{scripts/}} --- Standalone visualisation tools
\item \textbf{\texttt{tests/}} --- 178 unit and integration tests
\end{itemize}
...
The system accepts both raw KML (XML) and KMZ (zipped KML). Rather than depending on heavy geo-libraries such as \texttt{geopandas} or \texttt{GDAL}, we wrote a lightweight XML parser using Python's built-in \texttt{xml.etree.ElementTree}. This has several advantages:
...
Contour lines provide elevation values along irregular curves, not on a regular grid. To compute slope and flow direction we need a \textbf{Digital Elevation Model (DEM)} --- a regular grid of elevation values. We use \textbf{scipy's Radial Basis Function (RBF) interpolation} for this step.
...
Flow direction is the foundation of the catchment delineation algorithm. We use the \textbf{D8 (Deterministic 8-direction)} algorithm, which directs each cell to its single steepest downhill neighbour among the 8 cardinal/diagonal directions.
...
\subsection{No Pit Filling --- A Deliberate Choice}

Many hydrological workflows apply a \textbf{pit-filling} algorithm (e.g., Wang \& Liu, 2006) before computing flow direction, to eliminate flat areas and sinks. We explicitly \emph{do not} pit-fill, for the following reason:

\begin{quote}
\emph{Topographic depressions are exactly where we want to place ponds. Pit-filling artificially raises the elevation of those depressions so that water flows away from them, which causes the flow direction grid to route water \emph{out} of the best pond sites. This collapses the catchment of the top-ranked candidates to near-zero.}
\end{quote}

Without pit filling, the D8 grid preserves natural basins. The flow direction code 0 (no-flow) at pit cells is a correct representation: those cells have no downhill neighbour because they sit at the bottom of a local depression. The BFS catchment trace treats these correctly as terminal pour points.

\textbf{Consequence}: some candidates (those at pit cells in flat areas) have very small catchments because no surrounding cells drain into them under the strict D8 model. This is an accurate reflection of the terrain, not an algorithmic deficiency.
...
\midrule
\texttt{file} & \texttt{multipart/form-data} & KML or KMZ contour map \\
\bottomrule
\end{tabular}
...
\midrule
400 & Missing \texttt{file} field & No file key in multipart body \\
