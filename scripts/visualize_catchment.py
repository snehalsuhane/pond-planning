...
def main():
    ...
    try:
        with warnings.catch_warnings():
            ...
            # Use internal raster logic from delineate_catchment to get mask for visualisation
            from analysis.catchment import _REVERSE_D8
            ...
            for candidate in candidates["pond_candidates"]:
                ...
                candidate["catchment"] = delineate_catchment(hydro, pour_point, dem_result, epsg)
    ...
    for i, (candidate, mask) in enumerate(all_masks):
        ...
        if candidate["rank"] == 1:
            ax.scatter(px, py, color=color, marker='*', s=300, edgecolor='black', label=f"#{candidate['rank']} (Area: {candidate['catchment']['area_ha']:,.2f} ha)", zorder=10)
        else:
            ax.scatter(px, py, color=color, marker='o', s=100, edgecolor='black', label=f"#{candidate['rank']} (Area: {candidate['catchment']['area_ha']:,.2f} ha)", zorder=9)
    
    ax.set_title("Catchment Delineation for Top 10 Candidates")
